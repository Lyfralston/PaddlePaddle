...
# >>>>>>                    torch.nn.init.normal(self.hidden_dense.weight)  # done!
...
# >>>>>>        self.bert = transformers.AutoModel.from_pretrained(config.bert_name, cache_dir='./cache/')  # done!
        lstm_input_size += config.bert_hid_size
        self.dis_embs = paddle.nn.Embedding(num_embeddings=20, embedding_dim=config.dist_emb_size)
        self.reg_embs = paddle.nn.Embedding(num_embeddings=3, embedding_dim=config.type_emb_size)
...
# >>>>>>        _bert_embs = torch.masked_fill(_bert_embs, pieces2word.equal(y=0).unsqueeze(axis=-1), min_value)  # done!
        ...
        ### modified
# >>>>>>        packed_embs = torch.nn.utils.rnn.pack_padded_sequence(word_reps, sent_length.cpu(), batch_first=True, enforce_sorted=False)
        # packed_outs, (hidden, _) = self.encoder(packed_embs)
        packed_outs, (hidden, _) = self.encoder(word_reps)
        word_reps = packed_outs
# >>>>>>        word_reps, _ = torch.nn.utils.rnn.pad_packed_sequence(packed_outs, batch_first=True, total_length=sent_length.max())
        ### modified end
        cln = self.cln(packed_outs.unsqueeze(axis=2), word_reps)
        dis_emb = self.dis_embs(dist_inputs)
        tril_mask = paddle.tril(x=grid_mask2d.clone().astype(dtype='int64'))
        reg_inputs = tril_mask + grid_mask2d.clone().astype(dtype='int64')
        reg_emb = self.reg_embs(reg_inputs)
        conv_inputs = paddle.concat(x=[dis_emb, reg_emb, cln], axis=-1)
# >>>>>>        conv_inputs = torch.masked_fill(conv_inputs, grid_mask2d.equal(y=0).unsqueeze(axis=-1), 0.0)
        conv_inputs = masked_fill(conv_inputs, grid_mask2d.equal(y=0).unsqueeze(axis=-1), 0.0)
        conv_outputs = self.convLayer(conv_inputs)
# >>>>>>        conv_outputs = torch.masked_fill(conv_outputs, grid_mask2d.equal(y=0).unsqueeze(axis=-1), 0.0)
