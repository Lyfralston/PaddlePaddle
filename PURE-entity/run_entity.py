...
def save_model(model, args):
    ...
    model_to_save = model.bert_model.module if hasattr(model.bert_model,
        'module') else model.bert_model
    ...
    for i in range(len(batches)):
        ...
        for sample, preds in zip(batches[i], pred_ner):
            ...
            for span, pred in zip(sample['spans'], preds):
                span_id = '%s::%d::(%d,%d)' % (sample['doc_key'], sample[
                    'sentence_ix'], span[0] + off, span[1] + off)
                if pred == 0:
                    continue
                ner_result[k].append([span[0] + off, span[1] + off,
                    ner_id2label[pred]])
...
def evaluate(model, batches, tot_gold):
    ...
    logger.info('Cor: %d, Pred TOT: %d, Gold TOT: %d' % (cor, tot_pred,
        tot_gold))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_batch_size', type=int, default=32, help=
        'batch size during inference')
    parser.add_argument('--learning_rate', type=float, default=1e-05, help=
        'learning rate for the BERT encoder')
    ...
    parser.add_argument('--print_loss_step', type=int, default=100, help=
        'how often logging the loss value during training')
    parser.add_argument('--eval_per_epoch', type=int, default=1, help=
        'how often evaluating the trained model on dev set during training')
    ...
    parser.add_argument('--train_shuffle', action='store_true', help=
        'whether to train with randomly shuffled data')
    ...
    parser.add_argument('--dev_pred_filename', type=str, default=
        'ent_pred_dev.json', help='the prediction filename for the dev set')
    parser.add_argument('--test_pred_filename', type=str, default=
        'ent_pred_test.json', help='the prediction filename for the test set')
    parser.add_argument('--use_albert', action='store_true', help=
        'whether to use ALBERT model')
    parser.add_argument('--model', type=str, default='bert-base-uncased',
        help='the base model name (a huggingface model)')
    parser.add_argument('--bert_model_dir', type=str, default=None, help=
        'the base model directory')
    parser.add_argument('--seed', type=int, default=0)
    parser.add_argument('--gpu_device', type=int, default=7)
    parser.add_argument('--context_window', type=int, required=True,
        default=None, help='the context window size W for the entity model')
    ...
    if args.gpu_device > 0:
        paddle.device.set_device(device='gpu:' + str(args.gpu_device))
    ...
    if args.do_train:
        ...
        logger = get_logger(os.path.join(args.output_dir, 'eval.log'))
    ...
    dev_samples, dev_ner = convert_dataset_to_samples(dev_data, args.
        max_span_length, ner_label2id=ner_label2id, context_window=args.
        context_window)
    dev_batches = batchify(dev_samples, args.eval_batch_size)
    
    if args.do_train:
        train_data = Dataset(args.train_data)
        train_samples, train_ner = convert_dataset_to_samples(train_data,
            args.max_span_length, ner_label2id=ner_label2id, context_window
            =args.context_window)
        ...
        optimizer_grouped_parameters = [{'params': [p for n, p in
            param_optimizer if 'bert' in n]}, {'params': [p for n, p in
            param_optimizer if 'bert' not in n], 'lr': args.task_learning_rate}
            ]
        t_total = len(train_batches) * args.num_epoch
        # optimizer = paddle.optimizer.AdamW(parameters=optimizer_grouped_parameters, learning_rate=
        #     args.learning_rate, correct_bias=not args.bertadam)
        # scheduler = transformers.get_linear_schedule_with_warmup(optimizer, int(t_total * args.warmup_proportion), t_total)
        scheduler = paddle.optimizer.lr.LinearWarmup(learning_rate=args.learning_rate, 
    ...
    if args.do_eval:
        ...
        if args.eval_test:
            ...
            prediction_file = os.path.join(args.output_dir, args.
                test_pred_filename)
        else:
            test_data = Dataset(args.dev_data)
            prediction_file = os.path.join(args.output_dir, args.
                dev_pred_filename)
        test_samples, test_ner = convert_dataset_to_samples(test_data, args
        ...
        output_ner_predictions(model, test_batches, test_data, output_file=
            prediction_file)
