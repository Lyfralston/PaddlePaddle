...
def batchify(samples, batch_size):
    ...
    samples = [sample for i, sample in enumerate(samples) if i not in
        to_single_batch]
...
def convert_dataset_to_samples(dataset, max_span_length, ner_label2id=None,
    ...
    for c, doc in enumerate(dataset):
        ...
        for i, sent in enumerate(doc):
            ...
            sample = {'doc_key': doc._doc_key, 'sentence_ix': sent.sentence_ix}
            if context_window != 0 and len(sent.text) > context_window:
                logger.info('Long sentence: {} {}'.format(sample, len(sent.
                    text)))
            ...
            for i in range(len(sent.text)):
                for j in range(i, min(len(sent.text), i + max_span_length)):
                    sample['spans'].append((i + sent_start, j + sent_start,
                        j - i + 1))
                    span2id[i, j] = len(sample['spans']) - 1
                    if (i, j) not in sent_ner:
                        sample['spans_label'].append(0)
                    else:
                        sample['spans_label'].append(ner_label2id[sent_ner[
                            i, j]])
    ...
    logger.info(
        'Extracted %d samples from %d documents, with %d NER labels, %.3f avg input length, %d max length'
         % (len(samples), data_range[1] - data_range[0], num_ner,
