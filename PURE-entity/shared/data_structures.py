import sys
sys.path.append('/home/lyf/paddle/PURE-pd/utils')
import paddle_aux
import paddle
"""
...
def get_sentence_of_span(span, sentence_starts, doc_tokens):
    ...
    in_between = [(span[0] >= start and span[1] <= end) for start, end in
...
class Document:
    ...
    def __init__(self, js):
        ...
        entries = fields_to_batches(js, ['doc_key', 'clusters',
            'predicted_clusters', 'section_starts'])
        sentence_lengths = [len(entry['sentences']) for entry in entries]
        sentence_starts = np.cumsum(sentence_lengths)
        sentence_starts = np.roll(sentence_starts, 1)
        sentence_starts[0] = 0
        self.sentence_starts = sentence_starts
        self.sentences = [Sentence(entry, sentence_start, sentence_ix) for 
            sentence_ix, (entry, sentence_start) in enumerate(zip(entries,
            sentence_starts))]
        if 'clusters' in js:
            self.clusters = [Cluster(entry, i, self) for i, entry in
                enumerate(js['clusters'])]
        if 'predicted_clusters' in js:
            self.predicted_clusters = [Cluster(entry, i, self) for i, entry in
                enumerate(js['predicted_clusters'])]

    def __repr__(self):
        return '\n'.join([(str(i) + ': ' + ' '.join(sent.text)) for i, sent in
...
class Sentence:
    ...
    def __init__(self, entry, sentence_start, sentence_ix):
        ...
        if 'ner_flavor' in entry:
            self.ner = [NER(this_ner, self.text, sentence_start, flavor=
                this_flavor) for this_ner, this_flavor in zip(entry['ner'],
                entry['ner_flavor'])]
        elif 'ner' in entry:
            self.ner = [NER(this_ner, self.text, sentence_start) for
                this_ner in entry['ner']]
        if 'relations' in entry:
            self.relations = [Relation(this_relation, self.text,
                sentence_start) for this_relation in entry['relations']]
        if 'events' in entry:
            self.events = Events(entry['events'], self.text, sentence_start)
        if 'predicted_ner' in entry:
            self.predicted_ner = [NER(this_ner, self.text, sentence_start,
                flavor=None) for this_ner in entry['predicted_ner']]
        if 'predicted_relations' in entry:
            self.predicted_relations = [Relation(this_relation, self.text,
                sentence_start) for this_relation in entry[
                'predicted_relations']]
        if 'predicted_events' in entry:
            self.predicted_events = Events(entry['predicted_events'], self.
                text, sentence_start)
        if 'top_spans' in entry:
            self.top_spans = [NER(this_ner, self.text, sentence_start,
                flavor=None) for this_ner in entry['top_spans']]
...
class Span:
    ...
    def __eq__(self, other):
        return (self.span_doc == other.span_doc and self.span_sent == other
            .span_sent and self.text == other.text)
...
class Argument:
    ...
    def __repr__(self):
        return self.span.__repr__()[:-1
            ] + ', ' + self.event_type + ', ' + self.role + ')'
...
class Events:
    ...
    def __init__(self, events_json, text, sentence_start):
        self.event_list = [Event(this_event, text, sentence_start) for
            this_event in events_json]
        self.triggers = set([event.trigger for event in self.event_list])
        self.arguments = set([arg for event in self.event_list for arg in
            event.arguments])
    ...
    def span_matches(self, argument):
        return set([candidate for candidate in self.arguments if candidate.
            span.span_sent == argument.span.span_sent])

    def event_type_matches(self, argument):
        return set([candidate for candidate in self.span_matches(argument) if
            candidate.event_type == argument.event_type])

    def matches_except_event_type(self, argument):
        matched = [candidate for candidate in self.span_matches(argument) if
            candidate.event_type != argument.event_type and candidate.role ==
            argument.role]
        return set(matched)
...
class Cluster:
    ...
    def __init__(self, cluster, cluster_id, document):
        ...
        for entry in cluster:
            sentence_ix = get_sentence_of_span(entry, document.
                sentence_starts, document.n_tokens)
            sentence = document[sentence_ix]
            span = Span(entry[0], entry[1], sentence.text, sentence.
                sentence_start)
...
def evaluate_predictions(dataset):
    ...
    scores_ner = compute_f1(counts['ner_predicted'], counts['ner_gold'],
        counts['ner_matched'])
    scores_relations = compute_f1(counts['relations_predicted'], counts[
        'relations_gold'], counts['relations_matched'])
    scores_strict_relations = compute_f1(counts['relations_predicted'],
        counts['relations_gold'], counts['strict_relations_matched'])
    return dict(ner=scores_ner, relation=scores_relations, strict_relation=
        scores_strict_relations)
...
def analyze_relation_coverage(dataset):
    ...
    print('Coverage by top 0.4 spans: %.3f (%d / %d), #candidates: %d' % (
        ...
        npair_top))
    print('Overlap: %.3f (%d / %d)' % (nrel_overlap / nrel_gold * 100.0,
        nrel_overlap, nrel_gold))
