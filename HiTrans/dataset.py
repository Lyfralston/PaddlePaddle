...
class Vocabulary(object):
    ...
    def __init__(self):
        self.label2id = {'neutral': 0, 'surprise': 1, 'fear': 2, 'sadness':
            3, 'joy': 4, 'anger': 5, 'disgust': 6}
...
def load_data(input_max):
    ...
    def processing_data(path):
        ...
        for i, item in data.iterrows():
            utt_data = {'utterance': item['Utterance'], 'emotion': item[
                'Emotion'], 'speaker': item['Speaker']}
    ...
    return (processing_data('./data/train_sent_emo.csv'), processing_data(
        './data/dev_sent_emo.csv'), processing_data('./data/test_sent_emo.csv')
        ), vocab
