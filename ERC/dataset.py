...
class Vocabulary(object):
    ...
    def __init__(self):
        self.label2id = {'hap':0, 'sad':1, 'neu':2, 'ang':3, 'exc':4, 'fru':5}
...
def load_data(input_max):
    ...
    def processing_data(sent_dict, label_dict, speaker_dict, ids):
        dia_dict = defaultdict(list)
        for id in ids:
            for sent, label, speaker in zip(sent_dict[id], label_dict[id], speaker_dict[id]):
                utt_data = {
                    'utterance': sent,
                    'emotion': vocab.id2label[label],
                    'speaker': speaker
                }
                dia_dict[id].append(utt_data)
        dia_input = []
        cls_index = []
        emo_label = []
        spk_label = []
        for dia in dia_dict.values():
            ...
            for i in range(len(_len_index) - 1):
                ...
                if start < input_max * chucks < end:
                    ...
                    _len_index = np.cumsum([len(x) for x in _dia_input])
                    chucks += 1
            _dia_input = [x for utt in _dia_input for x in utt] + [tokenizer
                .sep_token_id]
            _cls_index = [i for i, x in enumerate(_dia_input) if x ==
    ...
    path = './data/IEMOCAP_features.pkl'
    ...
    return (processing_data(videoSentence, videoLabels, videoSpeakers, trainVid), 
            processing_data(videoSentence, videoLabels, videoSpeakers, testVid[0:int(0.5*len(testVid))]), 
            processing_data(videoSentence, videoLabels, videoSpeakers, testVid[int(0.5*len(testVid)):])), vocab
